...
static void __attribute__((noreturn)) cow_entry(struct Trapframe *tf) {
	...
	if(!(perm & PTE_COW)){
		user_panic("user/lib/fork.c/cow_entry: panic\n");
	}
...
static void duppage(u_int envid, u_int vpn) {
	...
	if ((perm & PTE_D) && !(perm & PTE_LIBRARY) && !(perm & PTE_COW)) {
		...
		if ((r = syscall_mem_map(0, (void *)addr, envid, (void *)addr, perm))) {
			user_panic("user/lib/fork.c/duppage: mapping child failure\n");
		}
		if ((r = syscall_mem_map(0, (void *)addr, 0, (void *)addr, perm))) {
			user_panic("user/lib/fork.c/duppage: mapping parent failure\n");
		}
	} else {
		if ((r = syscall_mem_map(0, (void *)addr, envid, (void *)addr, perm))) {
			user_panic("user/lib/fork.c/duppage: mapping child failure\n");
...
int fork(void) {
	...
	for(int i = VPN(UTEMP); i < VPN(USTACKTOP); i++) {
		if(vpd[i / 1024] & vpt[i] & PTE_V) {
			duppage(child, i);
		}
	}
